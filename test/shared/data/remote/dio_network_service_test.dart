...
void main() {
  ...
  group('POST request\n', () {
    test('perform POST request successfully', () async {
      fakeDio.response = Response(
        statusCode: 200,
        data: {},
        requestOptions: RequestOptions(path: ''),
  ...
  test('dio base options should have baseUrl and headers', () {
    expect(dioNetworkService.dioBaseOptions.baseUrl, dioNetworkService.baseUrl);
    expect(dioNetworkService.dioBaseOptions.headers, dioNetworkService.headers);
  });
  test('should update request header', () {
    expect(
      dioNetworkService.updateHeader({'token': 'randomString'}),
      updatedHeader,
    );
  });
}
